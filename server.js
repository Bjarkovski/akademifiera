...
// File to persist email -> session_id mappings
...
// Save customer email -> session_id after successful payment
app.post('/save-customer', async (req, res) => {
        const { email, sessionId } = req.body;

             if (!email || !sessionId) {
                         return res.status(400).json({ error: 'E-post och session_id krävs.' });
             }
        ...
        customers[email.toLowerCase().trim()] = sessionId;
        saveCustomers(customers);

             res.json({ success: true });
});

// Look up session_id by email
app.post('/verify-email', async (req, res) => {
        const { email } = req.body;
             ...
             const customers = loadCustomers();
        const sessionId = customers[email.toLowerCase().trim()];

             if (!sessionId) {
                         return res.status(404).json({ error: 'Ingen betalning hittades för denna e-postadress.' });
             }

             const paid = await verifyPayment(sessionId);
        if (!paid) {
                    return res.status(402).json({ error: 'Betalningen kunde inte verifieras.' });
        }
